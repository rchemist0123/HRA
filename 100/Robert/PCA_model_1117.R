...
disease_and_BA <- function(BA_data,dataset_for_cox,birth_data,disease_target, strings){
  ...
  #get disease at most 500year before from first medical checkup
  ba_mk_temp_tmtm <-ba_mk_temp_tm %>% filter(minmcage<=minage& minage<minmcage+500)
...
death_and_BA <-function(BA_data,dataset_for_cox,birth_data,death_code_excluded){
  ...
  #get dead at most 500 year before from first medical checkup
  ba_mk_temp_tmtm <-ba_mk_temp_tm %>% filter(minmcage<=mindage& mindage<minmcage+500)
...
disease_cox_fn <-function(BA_data,disease_target, strings){
  ...
  try( final_c <- c(rownames(summary(disease_cox_step)$coefficients), 'BA_CA','age') , silent = TRUE )
  try( final_c <- c(rownames(extractHR(disease_cox_step)), 'BA_CA','age'), silent = TRUE)
 ...
 #### cat('BA-CA exp(coef) : ' ,summary(disease_cox1)$coefficients['BA_CA',2], '\n')
 #### cat('BA-CA 95CI : ', exp(confint(disease_cox1))['BA_CA',],'\n')
 #### cat('BA-CA p : ' ,summary(disease_cox1)$coefficients['BA_CA',5], '\n')
 #### print('')
 #### cat('CA exp(coef) : ' ,summary(disease_cox1)$coefficients['age',2], '\n')
  ####cat('CA 95CI : ', exp(confint(disease_cox1))['age',],'\n')
 #### cat('CA p : ' ,summary(disease_cox1)$coefficients['age',5], '\n')
  
  cat('C-index : ', summary(disease_cox1)$concordance[1], '\n')
  coef_exp<-(exp(disease_cox1$coefficients))
  print('@@ all selected coef @@')
  p_coef<-data.frame(summary(disease_cox1)$coefficients[,5])
  result_coef<-(cbind(data.frame(coef_exp),data.frame(exp(confint(disease_cox1))),p_coef))
  colnames(result_coef)<-c('coef', '95% CI', '95% CI', 'p-value')
  print(result_coef)
  print('')
...
death_cox_fn<-function(BA_data,death_code_excluded){
  ...
  try( final_c <- c(rownames(summary(death_cox_step)$coefficients), 'BA_CA','age') , silent = TRUE )
  try( final_c <- c(rownames(extractHR(death_cox_step)), 'BA_CA','age'), silent = TRUE)
  ...
  print('@@ death @@')
 #### cat('using : ', final_coef, '\n')
 ### cat('BA-CA exp(coef) : ' ,summary(death_cox1)$coefficients['BA_CA',2], '\n')
 #### cat('BA-CA 95CI : ', exp(confint(death_cox1))['BA_CA',],'\n')
 #### cat('BA-CA p : ' ,summary(death_cox1)$coefficients['BA_CA',5], '\n')
 #### print('')
 #### cat('CA exp(coef) : ' ,summary(death_cox1)$coefficients['age',2], '\n')
 #### cat('CA 95CI : ', exp(confint(death_cox1))['age',],'\n')
 ### cat('CA p : ' ,summary(death_cox1)$coefficients['age',5], '\n')
  
  cat('C-index : ', summary(death_cox1)$concordance[1], '\n')
 
  coef_exp<-(exp(death_cox1$coefficients))
  print('@@ all selected coef @@')
  p_coef<-data.frame(summary(death_cox1)$coefficients[,5])
  result_coef<-(cbind(data.frame(coef_exp),data.frame(exp(confint(death_cox1))),p_coef))
  colnames(result_coef)<-c('coef', '95% CI', '95% CI', 'p-value')
  print(result_coef)
...
disease_xgb <-function(age_label,disease_target,strings, gender,selected_components, BA_TF){
  ...
  ####print("###### importance top3 #######")
  ####print(xgb.importance(colnames(xgb_train), model=ML_model)[c(1,2,3),])
  ####print("###### importance BA-CA, CA #######")
  ####print(xgb.importance(colnames(xgb_train), model=ML_model)%>%filter(Feature%in% c('BA_CA','CA')))
  ...
  print("###### AUROC #######")
  cat('# AUROC :' , result_ROC$auc)
  print('')
  ###
  print("###### PRROC #######")
  a<-pr.curve(pred_y, test_y )
  cat('# PRC :' , a[2]$auc.integral)
  cat('  # PRC for cross check : ' , PRAUC(y_pred = pred_y, y_true = test_y))
  cat("  # prevalence : ",sum(confusion_matrix[2,])/sum(confusion_matrix),"\n")
