...
BA_add_to_dataset_ann <-function(ann_model, all_data,components, dictionary, gender, corrected){
  if (corrected==F){
    component2 <- components[-c(1,2)]
    data_specific_gender_temp<-all_data[component2]
    data_specific_gender_temp['Constant_column']<-1
    ...
    test_result<-compute(ann_model,subset(data_specific_gender_temp_scaled, select=-age))
    sc_age_ann <- test_result$net.result
    age_ann<-sc_age_ann*sc_std['age']+sc_center['age']
    data_specific_gender_temp[,'age']<-age_ann
    data_specific_gender_temp['Constant_column']<-1
...
PCA_model_R_ann<-function(ann_model, data_12, age_label, gender,selected_components){
  #### 1. sex, age를 기준으로 데이터 로딩 
  ...
  #### 7. Correction
  corrected_BA_parameters <- corrected_BA(data_specific_gender=data_specific_gender, BA_parameters, model=model)
  data_with_BA_ann <- BA_add_to_dataset_ann(ann_model=ann_model, all_data=all_data, dictionary =corrected_BA_parameters, components=selected_components, corrected=T)
  write.csv(data_with_BA_ann, paste0('/userdata06/room058/working/result_pca_r/data_with_BA_',gender,'_',age_label,'_ann.csv'))
  colnames(corrected_BA_parameters)<-c('parameter', 'coef')
  write.csv(corrected_BA_parameters, paste0('/userdata06/room058/working/result_pca_r/corrected_biological_age_coef_',gender,'_',age_label,'_ann.csv'))
  return(corrected_BA_parameters)
}
...
death_and_dm <-function(annTF ,selected_components){
  if (annTF==TRUE){
  corrected_BA_parameters <-PCA_model_R_ann(ann_model=ann_model,data_12=data_12, age_label=age_label,gender=gender,selected_components=selected_components)
  BA_data<-read.csv( paste0('/userdata06/room058/working/result_pca_r/data_with_BA_',gender,'_',age_label,'_ann.csv'))
  ...
  death_cox1 <- coxph(Surv(time, event==1)~BA_CA, data=death_dab_train1,x=TRUE)
  summary(exp(-predict(death_cox1, type='expected', newdata=death_dab_test1)))
  ...
  predict_data1<- ifelse(exp(-predict(disease_cox1, type='expected', newdata=dab_test1))<0.989,1,0)
...
death_and_dm_origin <-function(annTF ,selected_components){
  if (annTF==TRUE){
    corrected_BA_parameters <-PCA_model_R_ann(ann_model=ann_model,data_12=data_12, age_label=age_label,gender=gender,selected_components=selected_components)
    BA_data<-read.csv( paste0('/userdata06/room058/working/result_pca_r/data_with_BA_',gender,'_',age_label,'_ann.csv'))
...
'ANN : disease in, Q_PHX_~, FBS, TCHOL out, ready to fill in'
ANN <- function(raw_g1e0915, SEED=12345, findlayer=FALSE){
  ...
  data_ann<-data_12_nodm
  set.seed(12345)
  split_dummy <- sample( nrow(data_ann), size= 0.7*nrow(data_ann))
  ann_train <- data_ann[split_dummy, ] 
  ann_test <- data_ann[-split_dummy, ] 
  ann_tr<-ann_train %>% select((selected_components))%>% select(-SEX)
  ann_te<-ann_test %>% select((selected_components))%>% select(-SEX)
  
  # install.packages("neuralnet")
  ann_total<-data_ann%>%select((selected_components))%>%select(-SEX)
  ann_tr<-scale(ann_tr)
  ann_te<-scale(ann_te)
  (colnames(ann_tr))
  # layer 확인
  if (findlayer==TRUE){
    for (x in 1:10){
      ann_model<-neuralnet(hidden=x,stepmax=1e7, data=ann_tr, formula=age~G1E_BMI+G1E_WGHT+G1E_WSTC+G1E_BP_SYS+
                             G1E_FBS+G1E_TG+G1E_HDL+G1E_HGB+G1E_SGOT+G1E_SGPT+eGFR )
      ann_model$result.matrix
      test_result<-compute(ann_model,subset(ann_te, select=-age))
      cat("hidden layer : " , x, "error : " , ann_model$result.matrix[1])
      print('')
    }
  }
  # use TOTAL DATA because we need accuracy to normal people 
  ann_model<-neuralnet(hidden=4,stepmax=1e7, data=ann_total, formula=age~G1E_BMI+G1E_WGHT+G1E_WSTC+G1E_BP_SYS+
                         G1E_FBS+G1E_TG+G1E_HDL+G1E_HGB+G1E_SGOT+G1E_SGPT+eGFR)
  test_result<-compute(ann_model, subset(ann_total, select=-age))
  # saveRDS(ann_model, file='working/pcadata/annmodel.rda')
  return(ann_model)
}
...
#ALL
selected_components <-c('SEX','age','G1E_BMI','G1E_WGHT','G1E_WSTC','G1E_BP_SYS','G1E_BP_DIA','G1E_FBS','G1E_TOT_CHOL','G1E_TG','G1E_HDL','G1E_LDL','G1E_HGB', 'G1E_URN_PROT','G1E_CRTN','G1E_SGOT','G1E_SGPT','G1E_GGT','eGFR')
#MEDIAGE
selected_components <- c('SEX','age','G1E_BMI','G1E_WSTC','G1E_BP_SYS','G1E_FBS','G1E_HDL','G1E_HGB','G1E_SGPT','eGFR')

# 500K 
file_name <- paste0('working/result_pca_r/mediage_demo_dataset_NHIS_unit_bigger_cohort_',age_label,'.csv')
...
death_and_dm(annTF=FALSE,selected_components)
...
data_specific_gender <- data_12 %>% filter(data_12$SEX==gender)
dataset_for_cox<-read.csv('working/result_pca_r/dataset_for_cox.csv')
death_and_dm_origin(annTF=FALSE,selected_components)
...
all_parameter <-c('G1E_BMI','G1E_WGHT','G1E_WSTC','G1E_BP_SYS','G1E_BP_DIA','G1E_FBS','G1E_TOT_CHOL','G1E_TG','G1E_HDL','G1E_LDL','G1E_HGB', 'G1E_URN_PROT','G1E_CRTN','G1E_SGOT','G1E_SGPT','G1E_GGT','eGFR','SMK','DRK','METS')
...
death_and_dm(annTF=TRUE,selected_components)
