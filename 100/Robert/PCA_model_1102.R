...
DB_parser <- function(raw_g1e0915,birth_data,folderpath, parameter){
  ...
  all_dataset<- all_dataset %>% filter(20<=age) %>% filter(15<=G1E_BMI & G1E_BMI<=50) %>% 
    filter(50<=G1E_WSTC&G1E_WSTC<=130) %>% filter(50<=G1E_BP_SYS&G1E_BP_SYS<=200) %>% 
    filter(40<=G1E_BP_DIA & G1E_BP_DIA<=150) %>% 
    filter(40<=G1E_FBS & G1E_FBS<=500) %>% 
    filter(30<=G1E_TOT_CHOL & G1E_TOT_CHOL<=400) %>% 
    filter(30<=G1E_TG & G1E_TG<=500) %>% 
    filter(20<=G1E_HDL & G1E_HDL<=300) %>% 
    filter(G1E_LDL <=500) %>% filter(10<=G1E_HGB&G1E_HGB<=20) %>% 
    filter(0.3<=G1E_CRTN&G1E_CRTN<=10) %>%
    filter(G1E_SGOT<=200) %>% filter(G1E_SGPT<=200) %>% filter(G1E_GGT<=200)  %>% filter(G1E_URN_PROT %in% c(0,1,2,3,4)) %>%
    filter(30<eGFR & eGFR<=200)%>% filter(PA < 1500)%>% filter(PA2 < 2250000)%>% filter(DRK < 20) %>% filter(DRK2 < 400)
  
  tempb<- count_id(all_dataset)
  print(paste('all_data:',tempa,'->who has outlierand NA result :',tempa-tempb))
  ...
  all_dataset0912_save<-all_dataset0912
  ...
  all_dataset0912<-all_dataset0912_save
  ...
  print(paste('60-70 :',count_id(all_dataset091260)))
...
PCA_model_R<-function(age_label, gender,selected_components, ML_TF=FALSE){
  ...
  components_all= c('SEX', 'age', 'G1E_HGHT', 'G1E_WGHT','G1E_BMI', 'G1E_WSTC', 'G1E_BP_SYS', 'G1E_BP_DIA', 'G1E_FBS','G1E_TOT_CHOL', 'G1E_TG', 'G1E_HDL', 'G1E_LDL', 'G1E_HGB','G1E_URN_PROT',
                    'G1E_CRTN', 'G1E_SGOT', 'G1E_SGPT', 'G1E_GGT', 'eGFR', 'DRK', 'DRK2', 'SMK', 'PA', 'PA2','familyhistory',
...
disease_cox_fn <-function(BA_data,disease_target, strings){
  ...
  for_save<-data.frame(C_index=summary(disease_cox1)$concordance[1], HR=summary(disease_cox1)$coefficients[2], CI=exp(confint(disease_cox1)))
  colnames(for_save)<-c('C_INDEX', 'HAZARD_RATIO', '95%_CI_LOW','95%_CI_HIGH')
  for_save_temp = data.frame(summary(disease_cox1)[9]$logtest[3])
  colnames(for_save_temp)<-'P_VALUE'
  for_save <- cbind(for_save,for_save_temp)
  rownames(for_save)<-'VALUE'
  write.csv(for_save, paste0('working/result_pca_r/cox_result_',disease_target,'_', gender,'_',age_label,'.csv'))
  print(for_save)
...
death_cox_fn<-function(BA_data,death_code_excluded){
  ...
  for_save<-data.frame(C_index=summary(death_cox1)$concordance[1], HR=summary(death_cox1)$coefficients[2], CI=exp(confint(death_cox1)))
  colnames(for_save)<-c('C_INDEX', 'HAZARD_RATIO', '95%_CI_LOW','95%_CI_HIGH')
  for_save_temp = data.frame(summary(death_cox1)[9]$logtest[3])
  colnames(for_save_temp)<-'P_VALUE'
  for_save <- cbind(for_save,for_save_temp)
  rownames(for_save)<-'VALUE'
  write.csv(for_save, paste0('working/result_pca_r/cox_result_death_',gender,'_',age_label,'.csv'))
  print(for_save)
  print('')
  
}

death_and_dm  <-function(dataset_for_cox,age_label, gender, ML_TF=FALSE,selected_components){
  file_name <- paste0('working/result_pca_r/dataset_0912_healthy_',age_label,'.csv')
  data_12 = read.csv(file_name)
...
disease  <-function(data_12,dataset_for_cox,age_label, gender, ML_TF=FALSE,selected_components){
  file_name <- paste0('working/result_pca_r/dataset_0912_healthy_',age_label,'.csv')
  data_12 = read.csv(file_name)
  data_specific_gender <- data_12 %>% filter(data_12$SEX==gender)
...
selected_components <- c('SEX','age','G1E_BMI','G1E_WSTC','G1E_BP_SYS','G1E_FBS','G1E_TOT_CHOL','G1E_HDL','G1E_HGB','G1E_SGPT','eGFR','DRK','DRK2','SMK','familyhistory')
#DrinkSmoke + burden
selected_components <- c('SEX','age','G1E_BMI','G1E_WSTC','G1E_BP_SYS','G1E_FBS','G1E_TOT_CHOL','G1E_HDL','G1E_HGB','G1E_SGPT','eGFR','DRK','DRK2','SMK','familyhistory','obesity_burden','bp_burden','fbs_burden','wc_burden','hdl_burden','tg_burden','pa_burden','drinking_burden','smoking_burden')
# 300K original
death_and_dm(dataset_for_cox=dataset_for_cox,age_label=age_label, gender=gender,selected_components=selected_components)
disease (data_12=data_12,dataset_for_cox=dataset_for_cox,age_label=age_label, gender=gender,selected_components=selected_components)
# ML 
death_and_dm(dataset_for_cox=dataset_for_cox,age_label=age_label, gender=gender,selected_components=selected_components,  ML_TF=TRUE)
disease (data_12=data_12,dataset_for_cox=dataset_for_cox,age_label=age_label, gender=gender,selected_components=selected_components,  ML_TF=TRUE)
...
#DrinkSmoke + familyhistory
selected_components <- c('SEX','age','G1E_BMI','G1E_WSTC','G1E_BP_SYS','G1E_FBS','G1E_TOT_CHOL','G1E_TG','G1E_HDL','G1E_SGPT','eGFR','DRK','DRK2','SMK','familyhistory')
#DrinkSmoke + burden
selected_components <- c('SEX','age','G1E_BMI','G1E_WSTC','G1E_BP_SYS','G1E_FBS','G1E_TOT_CHOL','G1E_TG','G1E_HDL','G1E_SGPT','eGFR','DRK','DRK2','SMK','familyhistory','obesity_burden','bp_burden','fbs_burden','wc_burden','hdl_burden','tg_burden','pa_burden','drinking_burden','smoking_burden')
